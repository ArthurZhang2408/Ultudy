...
/**
 * GET /api/tier2/sections/:chapterMarkdownId
 *
 * Get cached sections for a chapter markdown source
 * Returns instant results (~50ms) - no LLM call needed!
 * Returns: { sections: [{ sectionNumber, sectionName, sectionDescription }] }
 */
router.get('/sections/:chapterMarkdownId', async (req, res) => {
  try {
    const userId = req.userId || 'dev-user-001';
    const { chapterMarkdownId } = req.params;

    // Verify ownership
    const ownershipCheck = await queryRead(
      `SELECT id FROM chapter_markdown WHERE id = $1 AND owner_id = $2`,
      [chapterMarkdownId, userId]
    );
    ...
    console.log(`[tier2/sections] Retrieved ${result.rows.length} sections for chapter ${chapterMarkdownId}`);
    ...
    console.error('[tier2/sections] Error:', error);
